...
def calculate_t_field(res, th):
    ...
    # 浮動小数点数として計算
    th_float = float(th)
    vx, vy = float(v) * np.cos(th_float), float(v) * np.sin(th_float)
    ...
    for n in range(-10, 11):
        ...
        if np.any(mask):
            q_at_t = Q + vy * t_h
            hit = np.abs(q_at_t % float(M)) < (float(M) * 0.02)
    ...
    for m in range(-10, 11):
        ...
        if np.any(mask):
            p_at_t = P + vx * t_v
            hit = np.abs(p_at_t % float(L)) < (float(L) * 0.02)
...
if 'current_theta' not in st.session_state:
    st.session_state.current_theta = 0.78  # 最初から小数にする
...
# 手動スライダー
# エラー回避のため、value を明示的に float(st.session_state.current_theta) にする
theta_slider = st.sidebar.slider(
    "角度 θ (rad) の手動調整", 
    min_value=0.0, 
    max_value=float(2 * np.pi), 
    value=float(st.session_state.current_theta), 
    step=0.01
)

# 再生中ならスライダーを無視、停止中ならスライダーの値をセッションに保存
...
if st.session_state.playing:
    # 角度を更新（0.1ずつ加算）
    st.session_state.current_theta += 0.05
    if st.session_state.current_theta > 2 * np.pi:
        st.session_state.current_theta = 0.0
